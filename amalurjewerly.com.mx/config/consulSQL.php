...
class ejecutarSQL {
    public static function conectar(){
        ...
        /* Codificar la informacion de la base de datos a UTF8*/
        mysqli_set_charset($con,'utf8');
        return $con;  
    }
    public static function consultar($query) {
        if (!$consul = mysqli_query(ejecutarSQL::conectar(),$query)) {
            die(mysqli_error().'Error en la consulta SQL ejecutada '.mysqli_error());
        }
        return $consul;
    }  
}
...
class consultasSQL{
    public static function InsertSQL($tabla, $campos, $valores) {
        if (!$consul = ejecutarSQL::consultar("INSERT INTO $tabla ($campos) VALUES ($valores)")) {
            die("INSERT INTO $tabla ($campos) VALUES ($valores) Ha ocurrido un error al insertar los datos en la tabla $tabla ".mysqli_error());
        }
        return $consul;
    }
