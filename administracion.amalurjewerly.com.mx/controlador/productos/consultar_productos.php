...
$verDueno=ejecutarSQL::consultar("SELECT * from producto where nombre like '%$_POST[valor]%' or codigo like '%$_POST[valor]%') order by nombre asc limit 5");

echo '<div class="table-responsive" style="padding:1%;"><table class="table-bordered" style="text-align:center;width:100%;">
<tr><th>Codigo</th><th>Nombre</th><th> Costo</th><th>Tipo</th><th>Stock</th><th>Seleccionar</th></tr>';
                                ...
                                <td>'.$dueno['precio_costo'].'</td>
                                <td>'.$dueno['generico_patente'].'</td>
                                <td>'.$dueno['existencia'].'</td>';

                               ?>
                                <td><a class="bnt btn-success btn-circle btn-sm" title="seleccionar" style="color: white;" onclick="seleccionar_producto_entrada(<?php echo $dueno['id_producto'];?>);"><i class="fa fa-plus"></i></a></td></tr>
                               <?php
                                }	
